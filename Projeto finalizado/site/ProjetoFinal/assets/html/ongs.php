...
<html lang="pt-BR">
    ...
    <head>
        ...
        <title>Contato voluntário - ONG's</title>
    ...
    <body class="tela">
		...
		<br><br><br><br><br><br>
		<h2 id="tOng">Ajude as ONG's</h2><br><br><br><br>
		<div class='container'>
			<div class='row'>
				<div class="form-pesquisa">
					<form method="POST" class="row g-3">
						<div class="col-4">
							<input type="text" class="form-control align-center" id="pesquisa" name="pesquisar" placeholder="O que você procura?">
						</div>
						<div class="col-auto">
							<button type="submit" class="btn btn-primary mb-3"><i class="fa fa-search" aria-hidden="true"></i></button>
						</div>
					</form>
				</div>
				<?php
					if(!empty($_POST['pesquisar'])){
						...
						if($pesquisar == ""){
							...
							$getRequests = mysqli_query($conexao,"SELECT * FROM usuario u, ramo r WHERE r.id_usuario=u.id_usuario and tipo_usuario='2' AND nome_usuario LIKE '%$pesquisar%' LIMIT 5");
							$recebePesquisa=mysqli_num_rows($getRequests);
							if( $recebePesquisa == 0){
								echo "<div class='container'>";
								echo " <br><br><br><br><h3 id='erroPesquisa' style='color: red;' class='display-6'>Não encontrado!</h3>";
								echo "</div>";
							}else{
								echo "<br><br><br><br><h2 id='efetuePesquisa'>Resultado</h2><br>";
								echo "<div class='container'>";
								echo "<div class='row'>";
									while($aux = $getRequests->fetch_assoc()){
										echo "<div class='col-xs-12 col-sm- 6 col-md-6 col-lg-4 col-xl-4'>";
										echo "<div class='cartao' style='width: 22rem; height: 700px;'>";
											echo "<img class='card-img-top TamImg' src='../../../login/imgCarregadas/".$aux["imagem"]."'/>";
												echo "<div class='card-body'";
													echo "<h5 class='card-title tCard'>".$aux['nome_usuario']."</h5>";
													echo "<p class='card-text tContato'>Serviço que procura</p>";
													echo "<p class='card-text tInfos'>".$aux['ramo']."</p>";
													echo "<p class='card-text tContato'>Contato</p>";
													echo "<p class='card-text tInfos'>".$aux['telefone1']."</p>";
													echo "<p class='card-text tInfos'>".$aux['telefone2']."</p>";
													echo "<p class='card-text tInfos'>".$aux['email']."</p>";
													echo "<p class='card-text tContato'>Endereço</p>";
													echo "<p class='card-text tInfos'>Cidade</p>";
												echo"</div>";
											echo "</div>";
										echo "</div>"; 
									} 
				...
				<?php
					$recebeDDong = mysqli_query($conexao,"SELECT * FROM usuario u, ramo r  WHERE r.id_usuario=u.id_usuario AND tipo_usuario='2'");
					$recebe		 =mysqli_num_rows($recebeDDong); 
					if( $recebe == 0 ){
						echo "<div class='container'>";
						echo "  <h3 id='erroCad' class='display-5'>O site não possui cadastros!</h3>";
						echo "</div>";
					}else{
						while($aux = $recebeDDong->fetch_assoc()){
